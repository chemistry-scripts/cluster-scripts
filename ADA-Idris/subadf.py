...
Script adapted for Idris Ada
Last Update 2017-12-19 by Emmanuel Nicolas
...
def create_shlexnames(runvalues):
    ...
    shlexnames['dependencies'] = [shlex.quote(dep) for dep in runvalues['dependencies']]
    return shlexnames
...
def compute_memory(runvalues):
    ...
    Available: 3.5GB per core, ADF uses dynamic memory management.
    """
    if runvalues['memory'] is not None:
        memory = runvalues['memory']
    else:
        memory = 3500 * runvalues['cores']
...
def create_run_file(output, runvalues):
    ...
    for dependency in shlexnames['dependencies']:
