...
def get_options():
    ...
    parser.add_argument('-q', '--queue', type=str, required=True,
                        help="Queue on which the computation should run")
    ...
    runvalues = dict.fromkeys(['inputfile', 'cores', 'walltime', 'queue',
                               'memory', 'chk', 'oldchk', 'rwf',
                               'nproc_in_input', 'memory_in_input'])
    # Get values from parser
    runvalues['inputfile'] = os.path.basename(args.inputfile[0])
    runvalues['cores'] = args.nproc
    runvalues['walltime'] = args.walltime
    queue_list = ['small', 'medium', 'large', 'x_large']
    if args.queue not in queue_list:
        print("Invalid queue")
        print("Choose in " + " ".join(queue_list))
        sys.exit(2)
    else:
        runvalues['queue'] = args.queue
    if args.memory:
        runvalues['memory'] = args.memory
    else:
        runvalues['memory'] = False
...
def get_values_from_input_file(input_file, runvalues):
    """
        Get core/memory/etc values from input file, reading the Mem and
        NProcShared and other parameters
    """
...
def create_run_file(input_file, output, runvalues):
    ...
    out = ['#!/bin/bash\n',
           '#PBS -q ' + shlexnames['queue'] + '\n',
           '#PBS -l walltime=' + runvalues['walltime'] + '\n',
           '#PBS -l mem=' + str(memory) + '\n',
           '#PBS -j o7'
           ''
           ...
           '\n']
    out.extend(['# Setup Gaussian specific variables\n',
                'export g09root\n',
